...
- **外部知識の RAG 化**（Web 記事・ニュース原文・IR 資料全文・決算短信などアプリ外の
  情報をチャンク化して検索対象にすること）。理由:
  1. 原文の保存・チャンク化は全文複製にあたり、著作権方針（計画 08:
     タイトル・要旨・URL のみ保存）に反する
  2. 市場情報は陳腐化が速く、古い外部情報が検索で混入すると誤った文脈で
     AI が判定する事故（鮮度事故）が起きる。ニュースは「取得当日に判定して
     記録を残す」フロー型（計画 08）が正しく、ストック型の RAG に向かない
  3. 外部知識を根拠に AI が事実を超えた示唆（「割安と考えられます」等）を
     し始めるリスクが増える。ユーザー自身の過去の判断なら
     「あなたが以前こう決めた」という事実の範囲で閉じる
  - RAG に入れるのは「そのニュースに対してユーザー/AI がどう判断したか」
    （= ユーザー所有の判断記録）だけ
- **全ユーザー共有コーパス**（用語集・教育コンテンツ・一般的な投資知識などを
  全員が検索できる知識ベースとして RAG に入れること）。理由:
  1. 既存 RAG はユーザー所有前提で設計されており、`retrieveRagContext` は
     `userId` で絞ることが安全性の根幹。共有データを混ぜるには
     「user_id が null の行も返す」という例外をこの境界に作ることになり、
     バグ 1 つで他ユーザーのメモが漏れる箇所と同じコードパスが複雑化する
  2. 用語集は構造化済みで件数も少なく、既存 glossary 機能のキー参照で
     十分（ベクトル検索は不要な複雑さ）
  3. 将来本当に必要になったら、ユーザー RAG とはテーブルも検索関数も
     分離した読み取り専用の別基盤として設計すべきで、この計画の範囲外
