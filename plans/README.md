...
| 14 | [14-earnings-documents.md](14-earnings-documents.md) | 決算資料の取り込みと参照（documents 連携 + EDINET 数値） | 04（thesis-draft 注入分のみ） |
| 15 | [15-expert-knowledge-base.md](15-expert-knowledge-base.md) | 専門家解説の共有ナレッジ基盤（段階導入・条件付き） | 04（「わからない」接続分のみ） |
...
- **フェーズ E（連携と記憶）**: 11 → 12
- **フェーズ F（データと知識の拡張）**: 14 → 15（15 のフェーズ 2 は導入ゲート成立後のみ）

実装状況（2026-07-17 時点）:

- 01〜10: 実装済み（codex が並列ワークツリーで実装。phase-a〜d の 4 ブランチに分散、
  **統合マージ待ち**。price 基盤と run-metered-ai-call が複数ブランチに重複実装されて
  いるため、統合時に phase-a 版を正として整理する）
- 11〜15: 未着手（統合マージ完了後に、統合ブランチを起点に実装する）
