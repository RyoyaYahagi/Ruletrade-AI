...
export async function getAiDeveloperDashboard(userId: string) {
  ...
  const [logsResult, experimentsResult, payloadLoggingEnabled] = await Promise.all([
    db
      .from("ai_run_logs")
      .select(
  ...
  const runs = (logsResult.data ?? []).map((row: DatabaseRow) => ({
    id: String(row.id),
    ...
    safetyPassed: toNullableBoolean(row.safety_passed),
    ...
    inputPreview: payloadLoggingEnabled
      ...
      : null,
    outputPreview: payloadLoggingEnabled
  ...
  const experiments = (experimentsResult.data ?? []).map((row: DatabaseRow) => ({
    id: String(row.id),
  ...
  return {
    ...
    promptCatalog: AI_PROMPT_CATALOG.map((prompt) => ({
      ...prompt,
      runCount: runs.filter((run) => run.promptVersion === prompt.version).length,
      lastUsedAt:
        runs.find((run) => run.promptVersion === prompt.version)?.startedAt ?? null,
    })),
    runs,
    experiments,
  };
