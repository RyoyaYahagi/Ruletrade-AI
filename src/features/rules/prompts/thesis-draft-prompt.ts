export const THESIS_DRAFT_PROMPT_VERSION = "thesis-draft-v2";
...
仮説は一人称で書き、破れ条件は観測可能な事実にしてください。事実を参照した場合は、必ずS番号のソース参照と、本文の文から15文字以上をそのまま抜き出した原文引用を返してください。タイトル・見出し・ナビゲーション文言は引用にしないでください。
例: 主力事業の減収が2四半期続く。
...
export function buildThesisDraftPrompt(params: {
  ...
  return JSON.stringify({
    ...
    constraints: [
      ...
      "事実と推論を分ける",
      "参照したソースはS番号で示し、引用は入力された本文の文から15文字以上をそのまま抜き出す",
      "タイトル・見出し・ナビゲーション文言を引用にしない",
      "破れ条件は観測可能な事実にする",
      "破れ条件を4件返す",
