...
const schemaStatements = [
  ...
  `create table if not exists rule_ai_trace_records (
    id text primary key,
    user_id text not null,
    session_id text not null,
    question_id text,
    question_key text,
    ai_run_log_id text,
    thesis_research_run_id text,
    trace_type text not null,
    status text not null,
    answer_context_hash text not null,
    input_json text,
    output_json text,
    input_hash text not null,
  ...
  `create table if not exists rule_analytics_daily (
    id text primary key,
    user_id text not null,
    ...
    created_at text not null default (datetime('now')),
    updated_at text not null default (datetime('now')),
    unique (user_id, metric_date, question_key)
  )`,
  ...
  `create index if not exists idx_rule_funnel_events_session_occurred on rule_funnel_events(user_id, session_id, occurred_at desc)`,
  `create index if not exists idx_rule_funnel_events_question on rule_funnel_events(question_key, event_name, occurred_at desc)`,
  `create index if not exists idx_rule_funnel_events_expiry on rule_funnel_events(expires_at)`,
  `create index if not exists idx_rule_ai_trace_records_session on rule_ai_trace_records(user_id, session_id, created_at desc)`,
  `create index if not exists idx_rule_ai_trace_records_research on rule_ai_trace_records(user_id, thesis_research_run_id)`,
  `create index if not exists idx_rule_ai_trace_records_expiry on rule_ai_trace_records(expires_at)`,
  `create index if not exists idx_rule_analytics_daily_date on rule_analytics_daily(metric_date, question_key)`,
