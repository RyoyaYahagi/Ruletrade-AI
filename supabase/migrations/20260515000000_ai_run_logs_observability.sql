...
create table if not exists model_pricing_configs (
  ...
  input_cost_per_1m_tokens_usd numeric(10,6) not null default 0 check (
    input_cost_per_1m_tokens_usd >= 0
  ),
  output_cost_per_1m_tokens_usd numeric(10,6) not null default 0 check (
...
create index if not exists idx_model_pricing_configs_provider_model
on model_pricing_configs(provider, model)
where is_active = true;

create index if not exists idx_ai_run_logs_user_created
on ai_run_logs(user_id, created_at desc);

create index if not exists idx_ai_run_logs_task_created
on ai_run_logs(task_type, created_at desc);

create index if not exists idx_ai_run_logs_source
on ai_run_logs(source_type, source_id);

create index if not exists idx_ai_run_logs_session
on ai_run_logs(session_id, created_at desc);

create index if not exists idx_ai_run_log_events_run_created
