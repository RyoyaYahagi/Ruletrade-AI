...
test.describe("Privacy Settings Form", () => {
  test.beforeEach(async ({ page }) => {
    ...
    page.on("response", async (response) => {
      if (response.url().includes("/api/privacy/settings")) {
        ...
        console.log(
          `[API] ${response.request().method()} ${response.url()} => ${response.status()}: ${body}`,
        );
      }
    });
    ...
    await expect(page.getByTestId("privacy-settings-form")).toBeVisible();
  });

  test("user can toggle privacy settings and see save confirmation", async ({
    page,
  }) => {
    ...
    await expect(page.getByTestId("privacy-message")).toContainText(
      "設定を更新しました",
      { timeout: 5000 },
    );
  ...
  test("toggling allowRagIndexing updates state correctly", async ({
    page,
  }) => {
    ...
    await page.waitForResponse(
      (resp) =>
        resp.url().includes("/api/privacy/settings") &&
