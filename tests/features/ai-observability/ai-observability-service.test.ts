...
describe("AI observability service", () => {
  ...
  it("returns only the requesting user's runs and experiment notes", async () => {
    ...
    expect(dashboard.experiments[0]).toMatchObject({ title: "Aの実験", tags: ["a"] });
  ...
  async function loadService() {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "ruletrade-ai-observability-"));
    tempDirs.push(tempDir);
    process.env.SQLITE_DATABASE_PATH = path.join(tempDir, "test.sqlite");
    process.env.NODE_ENV = "test";

    const service = await import("@/features/ai/services/ai-observability-service");
